...
def rit_simulation(
    ...
    *,
    ...
    lam = float(parameters.get("lam", 0.30))
    rho = float(parameters.get("rho", 0.30))
    psi = float(parameters.get("psi", 14.0))
    ...
    for i in range(1, steps):
        ...
        # ---- delL ----
        delL = (stepsize * (mul * Larr[i-1] + g * d_val - fc1 * Larr[i-1]) - hits_now * (1.0 - SL) * Larr[i-1])
        #delL = (stepsize * ((mul - fc1) * (Larr[i-1] - Lstart) + g * d_val ) - hits_now * (1.0 - SL) * Larr[i-1])
        ...
        #delLG = stepsize * (mul * (LGarr[i-1] - LGstart) + (1.0 - g) * d_val) - hits_now * kRadL * LGarr[i-1]
